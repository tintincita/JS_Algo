...
// early age math concepts flashback
function divideWithoutRemainder (dividend, divisor = 1) {
    let temp = dividend / divisor;
    temp = temp.toString();
    let temp2 = temp.split(".");
    return(parseInt(temp2[0]))
}


for (i = 0; i<billValues.length; i++) {
    billQtts[i] = divideWithoutRemainder(carry, billValues[i]);
